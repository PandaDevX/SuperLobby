...
public class DelSpawnCommand implements CommandExecutor {
    ...
    public DelSpawnCommand(JavaPlugin plugin) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if(!player.hasPermission("sl.delspawn")) {
            ChatUtil.sendPlayerMessage(player, "You have no permission to do that");
            return true;
        }
        FileHandler.deleteLocation();
        ChatUtil.sendPlayerMessage(player, "&aSuccessfully deleted a spawn");
        return false;
    }
}