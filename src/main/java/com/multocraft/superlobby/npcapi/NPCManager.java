...
public class NPCManager {
    ...
    private void handleEntityClick(Player player, int entityId, NPCClickAction action) {
        registeredNPCs.stream()
                ...
                .forEach(npc -> Bukkit.getServer().getScheduler().runTaskLater(plugin, () -> {
                    ...
                    if(previouslyClickedNPC != null && previouslyClickedNPC.equals(npc)) return;
    ...
    public NPC newNPC(NPCOptions npcOptions) {
        ...
        switch (serverVersion) {
            case REFLECTED:
                npc = new NPC_Reflection(plugin, npcOptions);
                break;
            case v1_16_R3:
                npc = new NPC_v1_16_R3(plugin, npcOptions);
                break;
        }
        if (npc == null) {
